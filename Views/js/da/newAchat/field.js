...
export function createFieldAndAppendTo(
  ...
  field.required = ["artDesi", "dateFinSouhaite", "qteDem"].includes(fieldName);

  if (fieldName === "dateFinSouhaite") {
    field.value = dateFinSouhaitee;
  } else if (fieldName === "numeroLigne") {
    let newLine = parseInt(localStorage.getItem("daAchatNumLigneMax")) + 1; // Détermine le nouveau numéro de ligne
    localStorage.setItem("daAchatNumLigneMax", newLine); // Changer la valeur de newLine
...
export function createFieldAutocompleteAndAppendTo(
  ...
  // Sélection de l'élément cible
  let field = prototype.querySelector(`[id*="${fieldName}"]`);
  field.required = fieldName === "artDesi"; // champ requis

  // Génération des nouveaux IDs pour le spinner et le conteneur
  let baseId = field.id.replace(
...
export function getTheField(
  line,
  fieldName,
  prefixId = "demande_appro_achat_form_demandeApproParentLines"
) {
