...
namespace App\Service\genererPdf\dit\facture;
...
class GenererPdfFactureAValidation extends GeneratePdf
{
     ...
     */
    function GenererPdfFactureSoumisAValidation(DitFactureSoumisAValidationDto $dto, array $montantPdf, string $email,  string $estFactureConformAOr)
    {
        $pdf = new TCPDF();
        ...
        $pdf->setFont('helvetica', '', 10);
        $pdf->cell(50, 6, $dto->numeroFact, 0, 1, '', false, '', 0, false, 'T', 'M');
        $pdf->setAbsX(130);
        $pdf->setFont('helvetica', 'B', 10);
        $pdf->cell(30, 6, 'Numéro Devis :', 0, 0, '', false, '', 0, false, 'T', 'M');
        $pdf->setFont('helvetica', '', 10);
        $pdf->cell(0, 6, $dto->numDevis, 0, 0, '', false, '', 0, false, 'T', 'M');
        ...
        $pdf->setFont('helvetica', '', 10);
        $pdf->cell(50, 6, $dto->etatOr, 0, 1, '', false, '', 0, false, 'T', 'M');
        $pdf->setAbsX(130);
        $pdf->setFont('helvetica', 'B', 10);
        ...
        $pdf->setFont('helvetica', '', 10);
        $pdf->cell(50, 6, $dto->numeroSoumission, 0, 1, '', false, '', 0, false, 'T', 'M');
